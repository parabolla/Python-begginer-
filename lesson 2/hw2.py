...
c = 1  # итерации
strochka = 0  # переменная для хранения/сравнения значений
while c != a + 1:  # выполняем, пока количесво строк не сравняется с кол. итераций
    cal = (one // (10 ** (b - 1)) % 10)  # высчитываем первый знак
    if strochka >= cal:  # если переменная >= высчитываемой цифры, пропускаем присваение ее к новому значению
        c += 1
        b -= 1
        continue
    else:
        ...
        strochka = cal  # присваение переменной высчитываемого знака

print("Самая большая цифра ", strochka)

# Выполнение через for
per = int(input())
# x = per
z = len(strochka(per))  # подсчитываем кол. символов для range
one = 0
two = 0
